import { CVData, CVTheme } from "@/types/cv";

export const defaultThemeEs: CVTheme = {
  id: "professional-blue",
  name: "Professional Blue",
  colors: {
    primary: "#2563eb",
    secondary: "#1e40af",
    accent: "#3b82f6",
    background: "#ffffff",
    text: "#1f2937",
    muted: "#6b7280",
  },
  layout: "sidebar",
};

export const spanishCVData: CVData = {
  id: "mathyu-cv-2025-es",
  personalInfo: {
    id: "personal-1",
    fullName: "Mathyu Cardozo",
    title: "Tech Lead / Desarrollador Full Stack Sr",
    email: "xmathyu@gmail.com",
    phone: "+51 924-283-802",
    location: "Perú",
    linkedin: "mathyu-cardozo",
    github: "xMathyu",
    website: "https://mathyu-portfolio.vercel.app/",
    profileImage: "/mathyu-profile.jpg",
    summary:
      "Desarrollador Fullstack Sr con 6 años de experiencia, los últimos 3 años trabajando de forma remota para empresas de EE.UU. con tecnologías como Next.js, NestJS, Java Spring Boot, Azure y DevOps. Tengo experiencia trabajando en startups en sectores como Retail, Fintech y Proveedores SaaS. Me especializo en el desarrollo de soluciones complejas, principalmente en telemetría financiera, automatización retail, CI/CD y pruebas de calidad. Trabajo de forma práctica en DevSecOps y seguridad de aplicaciones: análisis estático y dinámico integrado en pipelines de CI/CD, remediación del OWASP Top 10, gestión de identidad y accesos con Keycloak, y testing de seguridad automatizado con Playwright cubriendo flujos autenticados, autorización y configuración de seguridad. Comprometido con la mejora continua y la construcción de software eficiente y escalable.",
  },
  experiences: [
    {
      id: "exp-0",
      position: "Experto en Ciencia de Datos / AI Tech Lead",
      company: "Entel",
      location: "Híbrido",
      startDate: "2025-11",
      endDate: "",
      isCurrentJob: true,
      description:
        "Entel es una de las mayores empresas de telecomunicaciones en Chile y Perú. Como AI Tech Lead de SinfonIA, lidero el desarrollo de una plataforma impulsada por IA para transcribir cientos de miles de llamadas de audio diarias utilizando Whisper X, detección de silencio, y generando insights para ayudar a los agentes a mejorar su rendimiento y reducir costos operativos en un 75%.",
      companyLogo: "/logos/entel.png",
      achievements: [
        "Liderando el equipo de IA como Tech Lead de SinfonIA, una plataforma de transcripción de voz a texto que procesa cientos de miles de archivos de audio diariamente",
        "Implementé Whisper X para transcripción de voz a texto de alta precisión con capacidades de detección de silencio",
        "Desarrollé un motor de insights impulsado por IA para analizar el rendimiento de los agentes y proporcionar recomendaciones accionables",
        "Logré una reducción de costos del 75% en comparación con la solución del proveedor anterior mediante implementación de IA propia",
        "Diseñé infraestructura serverless escalable usando AWS Lambda y SQS para procesar y encolar cargas masivas de audio",
        "Construí pruebas de seguridad automatizadas con Playwright que cubren flujos autenticados, manejo de sesión y autorización por roles de la plataforma, ejecutándose en cada build de CI",
        "Apliqué prácticas DevSecOps al pipeline de IA: análisis estático como gate previo al release y endurecimiento de los servicios que procesan audio sensible de llamadas",
      ],
      technologies: [
        "Python",
        "FastAPI",
        "Whisper X",
        "AWS",
        "Lambda",
        "SQS",
        "OpenAI",
        "Next.js",
        "React",
        "Machine Learning",
        "LLMs",
        "RAG",
        "AWS Bedrock",
        "Azure AI Foundry",
        "Speech-to-Text",
        "NLP",
        "Playwright",
        "DevSecOps",
        "SAST",
      ],
    },
    {
      id: "exp-1",
      position: "Líder Técnico",
      company: "309 Technology - USA",
      location: "Híbrido",
      startDate: "2025-04",
      endDate: "2025-11",
      isCurrentJob: false,
      description:
        "309 Technology es una empresa startup con sede en EE.UU. enfocada en soluciones tecnológicas innovadoras y aplicaciones impulsadas por IA.",
      companyLogo: "/logos/309-technology.svg",
      achievements: [
        "Liderando todo el equipo de desarrollo utilizando metodologías ágiles como Scrumban y Jira para la gestión de proyectos",
        "Diseñando y desarrollando aplicaciones con Next.js, integración de IA, Java y Kotlin con Spring Boot",
        "Gestionando infraestructura en la nube en AWS e implementando soluciones de machine learning",
        "Diseñando interfaces de usuario y experiencia de usuario con Figma coordinando con equipos multifuncionales",
        "Estableciendo estándares de desarrollo y buenas prácticas para el equipo de ingeniería",
      ],
      technologies: [
        "Next.js",
        "AI",
        "Java",
        "Kotlin",
        "Spring Boot",
        "AWS",
        "Machine Learning",
        "Figma",
        "Scrumban",
        "Jira",
        "React",
        "TypeScript",
        "Microservices",
      ],
    },
    {
      id: "exp-2",
      position: "Ingeniero de Software Sr",
      company: "Encora (Clientes: Interbank, Niubiz, Scotiabank) - Perú",
      location: "Remoto",
      startDate: "2023-10",
      endDate: "2025-04",
      isCurrentJob: false,
      description:
        "Encora es una empresa tecnológica global que provee soluciones innovadoras de ingeniería de software a empresas líderes en diversas industrias.",
      companyLogo: "/logos/encora.png",
      achievements: [
        "Desarrollé aplicaciones backend con Java Spring Boot, implementando programación reactiva con WebFlux y RxJava, y garantizando servicios financieros de alto rendimiento. Realicé pruebas unitarias con JUnit y Mockito, despliegues en Azure y AWS, y monitoreo de aplicaciones con New Relic.",
        "Contribuí al desarrollo frontend con Angular y Next.js, y apliqué prácticas DevOps usando SonarQube, Kiuwan y CheckStyle. En Scotiabank, trabajé con Java 8 y Eclipse enfocándome en soluciones financieras seguras y eficientes.",
        "Ejecuté análisis estático de seguridad (SonarQube, Kiuwan) como gate de release en servicios bancarios, triando hallazgos y remediando vulnerabilidades antes del despliegue",
        "Implementé autenticación y autorización con Keycloak (OAuth2 / OIDC, JWT), incluyendo ciclo de vida del token y control de acceso por roles en las APIs financieras",
      ],
      technologies: [
        "Java",
        "Spring Boot",
        "WebFlux",
        "RxJava",
        "DynamoDB",
        "Redis",
        "Keycloak",
        "Azure",
        "AWS",
        "Kubernetes",
        "New Relic",
        "Angular",
        "Next.js",
        "SonarQube",
        "Kiuwan",
        "CheckStyle",
        "Jira",
        "JUnit",
        "Mockito",
        "Eclipse",
      ],
    },
    {
      id: "exp-3",
      position: "Ingeniero de Software Sr",
      company: "Serverli - Florida USA",
      location: "Remoto",
      startDate: "2024-06",
      endDate: "2024-12",
      isCurrentJob: false,
      description:
        "Serverli es una plataforma para gestionar y monitorear sistemas de seguridad físicos e IoT, integrando video, control de acceso y dispositivos en tiempo real. Como Ingeniero de Software Senior, lideré el desarrollo frontend con Next.js y backend con Nest.js, gestionando entornos en Azure, mejorando arquitecturas y especializándome en aplicaciones de seguridad IoT.",
      companyLogo: "/logos/serverli.png",
      achievements: [
        "Lideré el desarrollo frontend con Next.js y el desarrollo backend con Nest.js, enfocándome en aplicaciones de seguridad IoT",
        "Gestioné entornos en Azure, mejorando las arquitecturas del sistema y optimizando los procesos de despliegue",
        "Me especialicé en desarrollar soluciones de seguridad IoT integrando vigilancia por video, control de acceso y monitoreo de dispositivos en tiempo real",
        "Endurecí la plataforma mediante testing de seguridad interno: probé el control de acceso entre roles, revisé código buscando problemas del OWASP Top 10 y remedié los hallazgos",
        "Automaticé la validación de configuración de seguridad (headers, CSP, flags de cookies, HTTPS) y convertí cada vulnerabilidad corregida en un test de regresión con Playwright",
      ],
      technologies: [
        "JavaScript",
        "TypeScript",
        "Next.js",
        "Figma",
        "NestJS",
        ".Net",
        "C#",
        "Python",
        "Azure",
        "Kubernetes",
        "Playwright",
        "OWASP Top 10",
        "DAST",
      ],
    },
    {
      id: "exp-4",
      position: "Ingeniero de Software",
      company: "MDP Consulting",
      location: "Híbrido",
      startDate: "2020-11",
      endDate: "2023-10",
      isCurrentJob: false,
      description:
        "MDP Consulting es un referente reconocido en servicios de TI, ofreciendo soluciones innovadoras para clientes en América Latina. Durante mi tiempo en la empresa, progresé a través de múltiples roles: Líder Técnico Backend, Desarrollador de IA, Administrador Cloud & DevOps, Scrum Master & Mentor, y Desarrollador Senior, trabajando en proyectos diversos que abarcan desarrollo backend, integración de IA, infraestructura cloud y liderazgo de equipos.",
      companyLogo: "/logos/mdp-consulting.png",
      achievements: [
        "Líder Técnico Backend: Lideré el proyecto Pacific Client usando Java Spring Boot con Java 11/17, implementando patrones de programación funcional y reactiva",
        "Desarrollador de IA: Desarrollé soluciones de IA para el cliente Reseñan Colombia usando APIs de OpenAI y tecnologías de machine learning",
        "Administrador Cloud & DevOps: Gestioné automatización de infraestructura y pipelines de despliegue en plataformas AWS, Azure y GCP",
        "Scrum Master & Mentor: Lideré el programa de bootcamp de MDP, brindando capacitación técnica y mentoría a desarrolladores junior",
        "Tech Lead: Gestioné proyectos de desarrollo full-stack con Memorandum, una empresa tecnológica griega, supervisando el desarrollo backend y frontend",
        "Desarrollador Senior: Entregué soluciones backend de alta calidad con Java Spring Boot y Node.js, y aplicaciones frontend con Angular, Ionic y React",
        "DevSecOps: Integré análisis estático y escaneo de seguridad en los pipelines de CI/CD que administraba en AWS, Azure y GCP, bloqueando releases con hallazgos sin resolver",
      ],
      technologies: [
        "Java",
        "Spring Boot",
        "Node.js",
        "Angular",
        "React",
        "Ionic",
        "Python",
        "OpenAI",
        "AWS",
        "Azure",
        "GCP",
        "Docker",
        "Kubernetes",
        "Jenkins",
        "Terraform",
        "SonarQube",
        "DevSecOps",
        "SAST",
        "TDD",
        "BDD",
      ],
    },
  ],
  education: [
    {
      id: "edu-1",
      degree: "Ing. de Software",
      institution: "Universidad Peruana de Ciencias Aplicadas",
      location: "Lima, Perú",
      startDate: "2020-03",
      endDate: "2024-11",
      gpa: "",
      description: "Licenciatura en Ingeniería de Software",
      coursework: [
        "Ingeniería de Software",
        "Estructuras de Datos y Algoritmos",
        "Sistemas de Bases de Datos",
        "Arquitectura de Software",
        "Redes de Computadoras",
        "Sistemas Operativos",
      ],
    },
    {
      id: "edu-2",
      degree: "Inglés Avanzado (ICPNA)",
      institution: "Instituto de Idiomas",
      location: "Lima, Perú",
      startDate: "2019-01",
      endDate: "2020-12",
      gpa: "",
      description: "Programa de certificación de inglés avanzado",
      coursework: [],
    },
  ],
  skills: [
    // Backend
    {
      id: "skill-1",
      name: "Java",
      level: "expert",
      category: "backend",
      icon: "coffee",
    },
    {
      id: "skill-2",
      name: "Spring Boot",
      level: "expert",
      category: "backend",
      icon: "spring",
    },
    {
      id: "skill-3",
      name: "Node.js",
      level: "advanced",
      category: "backend",
      icon: "nodejs",
    },
    {
      id: "skill-4",
      name: "NestJS",
      level: "advanced",
      category: "backend",
      icon: "nestjs",
    },
    {
      id: "skill-5",
      name: ".NET",
      level: "intermediate",
      category: "backend",
      icon: "dotnet",
    },
    {
      id: "skill-6",
      name: "WebFlux",
      level: "advanced",
      category: "backend",
      icon: "spring",
    },
    {
      id: "skill-7",
      name: "RxJava",
      level: "advanced",
      category: "backend",
      icon: "java",
    },
    {
      id: "skill-8",
      name: "Flask",
      level: "advanced",
      category: "backend",
      icon: "python",
    },
    {
      id: "skill-9",
      name: "Django",
      level: "intermediate",
      category: "backend",
      icon: "python",
    },
    // Frontend
    {
      id: "skill-10",
      name: "Next.js",
      level: "expert",
      category: "frontend",
      icon: "nextjs",
    },
    {
      id: "skill-11",
      name: "React",
      level: "expert",
      category: "frontend",
      icon: "react",
    },
    {
      id: "skill-12",
      name: "Angular",
      level: "advanced",
      category: "frontend",
      icon: "angular",
    },
    {
      id: "skill-13",
      name: "Ionic",
      level: "intermediate",
      category: "frontend",
      icon: "ionic",
    },
    {
      id: "skill-14",
      name: "JavaScript",
      level: "expert",
      category: "language",
      icon: "javascript",
    },
    {
      id: "skill-15",
      name: "TypeScript",
      level: "advanced",
      category: "language",
      icon: "typescript",
    },
    {
      id: "skill-16",
      name: "Tailwind CSS",
      level: "advanced",
      category: "frontend",
      icon: "css3",
    },
    {
      id: "skill-17",
      name: "Sass",
      level: "advanced",
      category: "frontend",
      icon: "sass",
    },
    // Lenguajes de Programación
    {
      id: "skill-18",
      name: "C#",
      level: "intermediate",
      category: "language",
      icon: "csharp",
    },
    {
      id: "skill-19",
      name: "Python",
      level: "advanced",
      category: "language",
      icon: "python",
    },
    {
      id: "skill-20",
      name: "Kotlin",
      level: "advanced",
      category: "language",
      icon: "kotlin",
    },
    // Bases de Datos
    {
      id: "skill-21",
      name: "MySQL",
      level: "advanced",
      category: "database",
      icon: "mysql",
    },
    {
      id: "skill-22",
      name: "PostgreSQL",
      level: "advanced",
      category: "database",
      icon: "postgresql",
    },
    {
      id: "skill-23",
      name: "MongoDB",
      level: "intermediate",
      category: "database",
      icon: "mongodb",
    },
    {
      id: "skill-24",
      name: "Redis",
      level: "advanced",
      category: "database",
      icon: "redis",
    },
    // Mensajería
    {
      id: "skill-25",
      name: "RabbitMQ",
      level: "intermediate",
      category: "tool",
      icon: "rabbitmq",
    },
    // Testing
    {
      id: "skill-26",
      name: "JUnit",
      level: "advanced",
      category: "testing",
      icon: "testing",
    },
    {
      id: "skill-27",
      name: "Mockito",
      level: "advanced",
      category: "testing",
      icon: "testing",
    },
    {
      id: "skill-28",
      name: "Jest",
      level: "advanced",
      category: "testing",
      icon: "testing",
    },
    {
      id: "skill-29",
      name: "Selenium",
      level: "intermediate",
      category: "testing",
      icon: "testing",
    },
    {
      id: "skill-30",
      name: "Postman",
      level: "advanced",
      category: "testing",
      icon: "postman",
    },
    {
      id: "skill-31",
      name: "Karate",
      level: "intermediate",
      category: "testing",
      icon: "testing",
    },
    // Cloud
    {
      id: "skill-32",
      name: "Azure",
      level: "advanced",
      category: "cloud",
      icon: "azure",
    },
    {
      id: "skill-33",
      name: "AWS",
      level: "advanced",
      category: "cloud",
      icon: "aws",
    },
    {
      id: "skill-34",
      name: "GCP",
      level: "intermediate",
      category: "cloud",
      icon: "gcp",
    },
    {
      id: "skill-45",
      name: "SQS",
      level: "intermediate",
      category: "cloud",
      icon: "aws",
    },
    // DevOps
    {
      id: "skill-35",
      name: "Docker",
      level: "advanced",
      category: "devops",
      icon: "docker",
    },
    {
      id: "skill-36",
      name: "Kubernetes",
      level: "intermediate",
      category: "devops",
      icon: "kubernetes",
    },
    {
      id: "skill-37",
      name: "Jenkins",
      level: "intermediate",
      category: "devops",
      icon: "jenkins",
    },
    {
      id: "skill-38",
      name: "Terraform",
      level: "intermediate",
      category: "devops",
      icon: "terraform",
    },
    {
      id: "skill-39",
      name: "GitHub",
      level: "expert",
      category: "devops",
      icon: "github",
    },
    {
      id: "skill-40",
      name: "New Relic",
      level: "intermediate",
      category: "devops",
      icon: "newrelic",
    },
    {
      id: "skill-41",
      name: "SonarQube",
      level: "intermediate",
      category: "security",
      icon: "sonarqube",
    },
    // Diseño y Herramientas
    {
      id: "skill-42",
      name: "Figma",
      level: "intermediate",
      category: "tool",
      icon: "figma",
    },
    // IA/ML
    {
      id: "skill-43",
      name: "Machine Learning",
      level: "intermediate",
      category: "ai",
      icon: "ai",
    },
    {
      id: "skill-44",
      name: "OpenAI API",
      level: "advanced",
      category: "ai",
      icon: "openai",
    },
    {
      id: "skill-45",
      name: "DevSecOps",
      level: "advanced",
      category: "security",
    },
    {
      id: "skill-46",
      name: "SAST",
      level: "advanced",
      category: "security",
    },
    {
      id: "skill-47",
      name: "DAST",
      level: "intermediate",
      category: "security",
    },
    {
      id: "skill-48",
      name: "OWASP Top 10",
      level: "advanced",
      category: "security",
    },
    {
      id: "skill-49",
      name: "Kiuwan",
      level: "intermediate",
      category: "security",
    },
    {
      id: "skill-50",
      name: "Keycloak",
      level: "advanced",
      category: "security",
    },
    {
      id: "skill-51",
      name: "OAuth2",
      level: "advanced",
      category: "security",
    },
    {
      id: "skill-52",
      name: "JWT",
      level: "advanced",
      category: "security",
    },
    {
      id: "skill-53",
      name: "Playwright",
      level: "advanced",
      category: "testing",
    },
    {
      id: "skill-54",
      name: "LLMs",
      level: "advanced",
      category: "ai",
    },
    {
      id: "skill-55",
      name: "RAG",
      level: "advanced",
      category: "ai",
    },
    {
      id: "skill-56",
      name: "Roboflow",
      level: "intermediate",
      category: "ai",
    },
    {
      id: "skill-57",
      name: "Computer Vision",
      level: "intermediate",
      category: "ai",
    },
    {
      id: "skill-58",
      name: "NLP",
      level: "advanced",
      category: "ai",
    },
    {
      id: "skill-59",
      name: "AWS Bedrock",
      level: "advanced",
      category: "ai",
    },
    {
      id: "skill-60",
      name: "Azure AI Foundry",
      level: "advanced",
      category: "ai",
    },
  ],
  projects: [
    {
      id: "proj-1",
      name: "Monyx - App de Finanzas Personales con IA",
      description:
        "Desarrollé una aplicación integral de finanzas personales impulsada por IA que ayuda a los usuarios a categorizar gastos, capturar recibos mediante reconocimiento fotográfico y recibir recomendaciones financieras inteligentes para una mejor gestión del dinero.",
      technologies: [
        "Next.js",
        "React",
        "TypeScript",
        "AI/ML",
        "OpenAI API",
        "Computer Vision",
        "Node.js",
        "PostgreSQL",
        "Tailwind CSS",
        "Supabase",
      ],
      startDate: "2024-08",
      endDate: "2025-06",
      url: "https://monyx.vercel.app",
      image: "",
      highlights: [
        "Implementé categorización de gastos impulsada por IA usando algoritmos de machine learning",
        "Construí un sistema de OCR y visión computacional para escaneo automático de recibos y extracción de datos",
        "Creé un asesor financiero inteligente con recomendaciones personalizadas",
        "Desarrollé seguimiento de gastos en tiempo real y funciones de gestión de presupuesto",
        "Logré un 95% de precisión en la categorización de gastos y reconocimiento de recibos",
      ],
    },
    {
      id: "proj-2",
      name: "Calarm - Alarmas inteligentes según tu agenda",
      description:
        "App nativa de iPhone publicada en la App Store que crea alarmas a partir de los eventos del calendario: en dos toques eliges el evento y cuántos minutos antes quieres despertar. Desarrollada en Swift y SwiftUI.",
      technologies: ["Swift", "SwiftUI", "iOS", "EventKit", "App Store"],
      startDate: "2025-01",
      endDate: "",
      url: "https://apps.apple.com/us/app/calarm-smart-alarms/id6772419323",
      image: "",
      highlights: [
        "Reduje poner una alarma a dos toques: elegir el evento del calendario y cuántos minutos antes debe sonar",
        "Eliminé la cuenta mental que la tarea obliga a hacer cada noche: a qué hora es la reunión, cuánto tardo en prepararme, cuánto en llegar",
        "Integré el acceso al calendario con EventKit, gestionando el modelo de permisos y los requisitos de privacidad que Apple exige para leer calendarios",
        "Desarrollada de forma nativa en Swift y SwiftUI, publicada y mantenida en la App Store",
      ],
    },
    {
      id: "proj-3",
      name: "I Selvatici - Talleres de cocina y personal chef",
      description:
        "Sitio bilingüe para un proyecto de Basilicata que ofrece talleres de cocina, orto terapia y costura en Castelgrande, además de servicio de personal chef en toda Italia. Construido con Next.js sobre Vercel y con reservas por WhatsApp.",
      technologies: [
        "Next.js",
        "React",
        "i18n",
        "SEO",
        "Vercel",
        "Responsive Design",
      ],
      startDate: "2025-06",
      endDate: "",
      url: "https://selvatici.vercel.app/it",
      image: "",
      highlights: [
        "Construí un sitio bilingüe (italiano e inglés) con rutas y metadatos por idioma",
        "Prerenderizado estático de las páginas para carga rápida y contenido indexable",
        "Usé WhatsApp como canal de reserva, evitando por completo un backend y un sistema de reservas",
        "Estructuré el contenido en torno a talleres, próximas fechas y productos locales",
      ],
    },
    {
      id: "proj-4",
      name: "Asociación Fa Meng Chuen - Escuela de artes marciales",
      description:
        "Sitio para una escuela de artes marciales chinas en Miraflores, Lima, que presenta sus programas de Tai Chi, Sanda y Taolu y convierte las visitas en reservas de primera clase por WhatsApp. Construido con Next.js sobre Vercel.",
      technologies: [
        "Next.js",
        "React",
        "SEO",
        "Vercel",
        "Responsive Design",
      ],
      startDate: "2025-03",
      endDate: "",
      url: "https://famengchuen.com/",
      image: "",
      highlights: [
        "Presenté las disciplinas y programas de la escuela (Tai Chi, Sanda, Taolu) como núcleo de la página",
        "Canalicé las inscripciones a primera clase por WhatsApp en lugar de un formulario con backend",
        "Configuré metadatos localizados y etiquetas Open Graph para es-PE en dominio propio",
        "Diseño responsivo pensado para el tráfico móvil que la escuela recibe desde redes sociales",
      ],
    },
  ],
  certifications: [
    {
      id: "cert-1",
      name: "Microsoft Certified: Azure AI Engineer Associate",
      issuer: "Microsoft",
      date: "2025-04",
      expirationDate: "",
      url: "",
      credentialId: "",
    },
    {
      id: "cert-2",
      name: "Scrum Fundamentals Certified (SFC)",
      issuer: "SCRUMstudy - Accreditation Body for Scrum and Agile",
      date: "2024-04",
      expirationDate: "",
      url: "",
      credentialId: "1029346",
    },
    {
      id: "cert-3",
      name: "Ganador Hackathon IA - Izipay",
      issuer: "Izipay",
      date: "",
      expirationDate: "",
      url: "",
      credentialId: "",
    },
    {
      id: "cert-4",
      name: "Ganador Hackathon IA - BCP",
      issuer: "Banco de Crédito del Perú",
      date: "",
      expirationDate: "",
      url: "",
      credentialId: "",
    },
  ],
  languages: [
    { id: "lang-1", name: "Inglés Profesional", proficiency: "professional" },
    { id: "lang-2", name: "Español Nativo", proficiency: "native" },
    { id: "lang-3", name: "Portugués Básico", proficiency: "basic" },
  ],
  theme: defaultThemeEs,
  sections: {
    showPersonalInfo: true,
    showExperience: true,
    showEducation: true,
    showSkills: true,
    showProjects: true,
    showCertifications: true,
    showLanguages: true,
  },
  createdAt: "2025-01-01T00:00:00.000Z",
  updatedAt: "2025-01-01T00:00:00.000Z",
};
